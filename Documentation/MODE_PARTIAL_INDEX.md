...
> **2026-08-20 (EP-5):** rows below updated for the micro-package merges — presets folded into
> `behavior/ClassPreset.java`, DirectorAI+NameGenerator → `behavior/Director.java`, BotProfile →
> `core/BotProfile.java`, humanize+imperfection low-traffic classes → `behavior/humanize/Humanization.java`
> (nested). Package column now shows post-EP-3 paths for the merged rows; other rows still list the
> pre-EP-3 `phase0/*` layout and are pending a full re-triage.

| Package (post-EP-3) | Files | Count | Action hint (see S10-T08) |
|---|---|---|---|
| `behavior/` (brain row, EP-5) | BotBrain.java (was Phase0Brain), BotState.java, ClassPreset.java (absorbed ArcherPreset/FighterPreset/MagePreset/HealerPreset/BufferPreset/PresetFactory), StateMachine.java | 4 | re-verify with tests -> COMPLETE |
| `core/` (cabinet row, EP-5) | BotProfile.java | 1 | re-verify with tests -> COMPLETE |
| `behavior/` (director row, EP-5) | Director.java (DirectorAI + NameGenerator merged) | 1 | re-verify with tests -> COMPLETE |
| `behavior/humanize` (EP-5) | AntiDetectionEngine.java, Humanization.java (nested: HumanizedRandom, BehavioralFingerprint, SessionVariance, ImperfectionInjector + ReactionDelay/AFKModule), InputRandomizer.java, TimingJitter.java | 4 | re-verify with tests -> COMPLETE |
