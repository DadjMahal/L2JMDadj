...
/**
 * Composes the behavior modules with the real, proven engine — CombatAI,
 ...
 * CoreWiring (which only sends proven frames).
 *
...
public final class EngineDriver {
    private static final Logger LOGGER = Logger.getLogger(EngineDriver.class.getName());
    private static final long TICK_INTERVAL_MS = 300;
    private static long lastAdviceLogMs = 0;

    public static void main(String[] args) throws Exception {
        if (args.length < 3) {
            System.err.println("Usage: EngineDriver <account> <password> <charId> [host] [gamePort]");
            return;
        }
        ...
        if (!loggedIn) {
            LOGGER.severe("[EngineDriver] LoginServer auth failed for " + account);
            return;
        }
        ...
        if (!entered) {
            LOGGER.severe("[EngineDriver] GameServer enter-world failed for " + account);
            return;
        }
        gs.startReader();
        ...
        EngineWiring integration = player.getCombatAI().getBehaviorIntegration();

        LOGGER.info("[EngineDriver] " + account + " entered world, starting decision loop");

        while (true) {
            BotSnapshot snapshot = BotSnapshot.from(account, logger);

            String advice = integration != null ? integration.inventoryAdvice(snapshot) : null;
            if (advice != null && (System.currentTimeMillis() - lastAdviceLogMs) > 60_000) {
                LOGGER.info("[EngineDriver] " + account + " " + advice);
                lastAdviceLogMs = System.currentTimeMillis();
            }

            if (snapshot.hpCurrent <= 0) {
                String deathNote = integration != null && EngineConfig.getInstance().isDeathRecoveryEnabled()
                    ? integration.deathRecoveryStatus() : null;
                LOGGER.info("[EngineDriver] " + account + " STATE dead, pausing actions"
            ...
            LOGGER.fine("[EngineDriver] " + account + " STATE hp=" + snapshot.hpCurrent + "/" + snapshot.hpMax
            ...
            switch (decision.getAction()) {
                ...
                case USE_SKILL:
                    // Logged as SKIP-UNPROVEN inside CoreWiring/CombatFramePlanner —
                ...
                default:
                    ...
                    // old GameStateMirror.
                    int newTarget = (integration != null && EngineConfig.getInstance().isTargetingEnabled())
                        ? integration.selectTarget() : targetSelector.selectTarget();
                    if (newTarget != 0) {
            ...
            Thread.sleep(TICK_INTERVAL_MS + (integration != null ? integration.reactionDelayMs() : 0));
        }
    }
}
