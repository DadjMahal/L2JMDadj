...
## Open: **WPT-27 DONE-PUSHED `514f05c5`** (quest telemetry — QUEST_LIST 0x80 journal parse + v1 `"quests"` key). **WPT-21 DONE-PUSHED `e2eaa6a4` + VERIFIED LIVE** (movement-ack `/telemetry` route serving `MoveTelemetry.report()` returned per-bot evidence for all 5 bots on a fresh 2026-08-12 run). All other WPT tasks DONE-PUSHED. **TIM-001 HIGH — NOT resolved**: live-verified re-run proved far-travel ATTEMPTED (H2 ✓: ai_combat_04 3 far HOPs ~21391u, degenerate=0/3) but **H1 ✗ / H5 ✗** — `serverMoved=0`, `gameserver.characters` pos/exp identical before/after → movement not persisted (single far HOP exceeds 9900u per-move cap; `ZoneRouter.nextHop()` short multi-hop is the next step). See `RuntimeLogs/2026-08-12-tim001-evidence-run.md`.
